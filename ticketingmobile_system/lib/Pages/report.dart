...
class _AdminReportState extends State<AdminReport> {
  ...
  Future<bool> sendReportData(BuildContext context, int index) async {
    ...
    try {
      ...
      if (report != null && report['sent'] != true) {
  ...
  void handleReportSending(BuildContext context) async {
    final reportBox = await Hive.openBox('reports');
    final totalReports = reportBox.length;

    List<String> successReports = [];
    List<String> failedReports = [];
    List<String> alreadySentReports = [];

    for (int i = 0; i < totalReports; i++) {
      final report = reportBox.getAt(i);
      if (report != null &&
          (report['sent'] == null || report['sent'] == false)) {
        String reportIdentifier =
            'Report on ${report['date']} at ${report['time']}';
        print('Sending $reportIdentifier...');
        bool isSent = await sendReportData(context, i);

        if (isSent) {
          successReports.add(reportIdentifier);
        } else {
          failedReports.add(reportIdentifier);
        }
        await Future.delayed(Duration(seconds: 1));
      } else if (report != null && report['sent'] == true) {
        String reportIdentifier =
            'Report on ${report['date']} at ${report['time']}';
        alreadySentReports.add(reportIdentifier);
      }
    }

    String summaryMessage = '';
    if (successReports.isNotEmpty) {
      summaryMessage += 'Sent: ${successReports.join(', ')}.\n';
    }
    if (failedReports.isNotEmpty) {
      summaryMessage += 'Failed to send: ${failedReports.join(', ')}.\n';
    }
    if (alreadySentReports.isNotEmpty) {
      summaryMessage += 'Already sent: ${alreadySentReports.join(', ')}.\n';
    }

    if (summaryMessage.isEmpty) {
      summaryMessage = 'No new reports were sent.';
    }
                                ...
                                onPressed: () async {
                                  ...
                                  for (int i = 0; i < totalReports; i++) {
                                    ...
                                    if (report != null &&
                                      ...
                                      print('Sending $reportIdentifier...');
                                      bool isSent =
                                          await sendReportData(context, i);
                                  ...
                                  String overallResult = failedReports.isEmpty
                                      ? 'All reports were sent successfully.'
                                      : 'Some reports failed to send.';

                                  await showDialog(
                                    context: context,
                                    builder: (BuildContext context) {
                                      return AlertDialog(
                                        title: Text('Overall Report Status'),
                                        content: Text(overallResult),
                                        actions: [
                                          TextButton(
                                            child: Text('OK'),
                                            onPressed: () {
                                  ...
                                  if (successReports.isNotEmpty) {
                                    summaryMessage +=
                                        'Sent: ${successReports.join(', ')}.\n';
                                  }
                                  if (failedReports.isNotEmpty) {
                                    summaryMessage +=
                                        'Failed to send: ${failedReports.join(', ')}.\n';
                                  }
                                  if (alreadySentReports.isNotEmpty) {
                                    summaryMessage +=
                                        'Already sent: ${alreadySentReports.join(', ')}.\n';
                                  }

                                  if (summaryMessage.isEmpty) {
                                    summaryMessage =
                                        'No new reports were sent.';
                                  }
